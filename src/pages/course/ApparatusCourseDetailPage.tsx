...
export function ApparatusCourseDetailPage() {
  ...
  return (
    <Layout title={course.title} showBack>
      <div className="flex flex-col gap-4 pb-10">

        {/* Header */}
        <div className={`bg-gradient-to-br ${course.gradient} text-white px-4 pt-5 pb-6`}>
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-widest text-white/50 mb-1">Apparatus CPD</p>
          ...
          {loading ? (
            ...
            course.modules.map((mod, index) => {
              ...
              return (
                <button
                  key={mod.id}
                  onClick={() => unlocked && navigate(`/course/apparatus/${courseSlug}/${mod.id}`)}
                  disabled={!unlocked}
