...
import { CheckCircle, Lock, Clock, ChevronRight, Award } from 'lucide-react'
import { supabase } from '../../lib/supabase'
import { useAuth } from '../../contexts/AuthContext'
import { Layout } from '../../components/layout/Layout'
import { AREA_LEAD_COURSE } from '../../data/areaLeadCourse'

export function AreaLeadCoursePage() {
  ...
  return (
    <Layout title="Senior Lead Course" showBack>
      <div className="flex flex-col gap-4 pb-10">

        {/* Header */}
        <div className="bg-gradient-to-br from-violet-700 to-purple-900 text-white px-4 pt-5 pb-6">
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-widest text-violet-300 mb-1">UKAG Development Programme</p>
        ...
        <div className="px-4 flex flex-col gap-3">
          ...
          <div className="bg-violet-50 border border-violet-100 rounded-2xl p-4">
            ...
            <div className="mt-3 pt-3 border-t border-violet-100 flex items-center justify-between text-xs text-gray-400">
              <span>Assessment: Online Learning + Workshop Attendance</span>
            </div>
          </div>
          ...
          {certificate ? (
            <div className="bg-gradient-to-br from-violet-700 to-purple-900 rounded-2xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="font-extrabold text-white text-sm">UKAG Senior Lead Award Earned</p>
                <p className="text-xs text-white/60">
                  Completed {new Date(certificate.completed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
                <p className="text-xs text-violet-300 font-semibold mt-0.5">Visible on your profile</p>
              </div>
            </div>
          ) : allDone ? (
          ...
          {loading ? (
            ...
            course.modules.map((mod, index) => {
              ...
              return (
                <button
                  key={mod.id}
                  onClick={() => unlocked && navigate(`/course/area-lead/${mod.id}`)}
                  disabled={!unlocked}
                  ...
                  } ${done ? 'border-violet-200' : 'border-gray-100'}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {done && <span className="text-[10px] font-bold bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded-full">Complete</span>}
                    </div>
          ...
          <p className="text-xs text-gray-400 text-center pt-2">
            Complete each module in order. The UKAG Senior Lead Award certificate is awarded on completion and saved to your profile.
          </p>
        </div>
      </div>
    </Layout>
