...
// ─── ID Card (module-level, no inputs) ────────────────────────────────────
...
function CertRow({ cert, onDelete, onDownload }: CertRowProps) {
...
export function CoachProfilePage() {
  ...
  const [photoUrl, setPhotoUrl] = useState<string | undefined>(profile?.photo_url)
  const [fields, setFields] = useState({
    phone: profile?.phone ?? '',
    dbs_number: profile?.dbs_number ?? '',
    dbs_expiry: profile?.dbs_expiry ?? '',
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `photos/${profile.id}/profile.${ext}`
    await supabase.storage.from('coach-files').remove([path])
  ...
  async function addCert() {
    if (!profile || !certForm.title.trim()) return
    setUploadingCert(true)

    let filePath: string | null = null
    if (certFile) {
      const ext = certFile.name.split('.').pop()
      filePath = `certs/${profile.id}/${Date.now()}.${ext}`
  ...
  async function deleteCert(id: string) {
    ...
    if (!cert) return
    if (cert.file_path) {
      await supabase.storage.from('coach-files').remove([cert.file_path])
    }
      ...
      <div className="px-4 pt-6 flex flex-col gap-5 pb-8">
        ...
        <Card>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Tap to upload a photo from your camera roll</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Input
              ...
              onChange={e => setFields({ ...fields, dbs_number: e.target.value })}
            />
            <Input
              label="DBS Expiry Date"
              type="date"
              value={fields.dbs_expiry}
              onChange={e => setFields({ ...fields, dbs_expiry: e.target.value })}
            />
            <Input
              label="Safeguarding Certificate Expiry"
              type="date"
              value={fields.safeguarding_expiry}
              onChange={e => setFields({ ...fields, safeguarding_expiry: e.target.value })}
            />
            <Input
              label="First Aid Certificate Expiry"
              type="date"
              value={fields.first_aid_expiry}
              onChange={e => setFields({ ...fields, first_aid_expiry: e.target.value })}
            />
        ...
        <Card>
          ...
          {addingCert && (
            <div className="flex flex-col gap-3 mb-4 pb-4 border-b border-gray-100">
              <Input
                ...
                placeholder="e.g. UKAG Gymnastics L2"
              ...
              <div className="flex flex-col gap-1">
                ...
                <input ref={certFileInputRef} type="file" className="hidden" onChange={e => setCertFile(e.target.files?.[0] ?? null)} />
                <button
          ...
          {certs.length === 0 && !addingCert ? (
            ...
            certs.map(cert => (
              <CertRow key={cert.id} cert={cert} onDelete={deleteCert} onDownload={downloadCert} />
            ))
          )}
        </Card>
      </div>
