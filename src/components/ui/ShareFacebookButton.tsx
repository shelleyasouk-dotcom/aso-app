interface Props {
  url?: string
  hashtag?: string
  label?: string
  className?: string
}

export function ShareFacebookButton({
  url,
  hashtag = 'ActiveSchool',
  label = 'Share on Facebook',
  className = '',
}: Props) {
  function share() {
    const shareUrl = url ?? window.location.href
    const fbUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&hashtag=${encodeURIComponent('#' + hashtag)}`
    window.open(fbUrl, '_blank', 'width=600,height=500,noopener,noreferrer')
  }

  return (
    <button
      onClick={share}
      className={`inline-flex items-center gap-2 bg-[#1877F2] hover:bg-[#166FE5] active:bg-[#1464D2] text-white font-bold text-sm px-5 py-2.5 rounded-xl transition-colors ${className}`}
    >
      {/* Facebook f logo */}
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.791-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.97h-1.513c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/>
      </svg>
      {label}
    </button>
  )
}
