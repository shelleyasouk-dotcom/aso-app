-- =============================================================================
-- ASO Onboarding — Quiz Bank Seed  (rewritten: one INSERT per question)
-- Source: Coach Work Handbook v1.0 + Operations Manual v1.0
-- Run after: onboarding_seed_content.sql
-- Idempotent: unique index + ON CONFLICT DO NOTHING per statement
-- correct_index is 0-based and is NEVER returned to the browser.
-- =============================================================================

-- Unique index (idempotent — safe to re-run)
CREATE UNIQUE INDEX IF NOT EXISTS uq_quiz_bank_task_question
  ON onboarding_quiz_banks (task_id, question);
...
VALUES (
  ...
  2, 'Write down exactly what was said as soon as the session ends, word for word. Delay leads to inaccuracy and the written record must capture the child''s exact words.', 'disclosure', 'medium'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  '["The child''s parent or guardian","Your Lead Coach or Area Lead","Naima directly, bypassing the Area Lead","The school headteacher"]'::jsonb,
...
VALUES (
  ...
  1, 'The staff WhatsApp group is for work communication only. Do not share personal content, session photos, or children''s details in personal chats.', 'communication', 'easy'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  2, 'Never turn a child away. Contact your Area Lead immediately. Record the child''s name, time of arrival, and who brought them. Do not allow participation until your Area Lead confirms.', 'register', 'medium'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  1, 'Ask for the adult''s name and check it against the register. No name, no go. If they are not on the contact list, call your Area Lead before releasing the child.', 'dismissal', 'medium'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  2, 'Stay with the child, never leave them alone. Then check the register for the parent''s emergency contact and call them directly. If no answer, call the secondary contact, then your Area Lead.', 'late_collection', 'medium'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  1, 'Always check the register before making any decision about snacks. Never assume you know a child''s allergies from memory. Always check the register each time.', 'allergy', 'easy'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  1, 'You cannot share session photos or children''s information via personal channels. All content from sessions must go through ASO official channels only.', 'social_media', 'easy'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  2, 'The staff WhatsApp group is for work communication only: confirming cover, sharing session updates, and passing on information. Not for social content or children''s information.', 'whatsapp', 'easy'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  2, 'All staff must hold an Enhanced DBS certificate including the children''s barred list check. No exceptions. No member of staff may work independently with children without a confirmed valid DBS.', 'dbs', 'easy'
) ON CONFLICT (task_id, question) DO NOTHING;
...
VALUES (
  ...
  2, 'Never turn a child away. Contact your Area Lead immediately. Record the child''s name, time of arrival, and who brought them. Do not allow participation until your Area Lead confirms.', 'sop', 'medium'
) ON CONFLICT (task_id, question) DO NOTHING;
